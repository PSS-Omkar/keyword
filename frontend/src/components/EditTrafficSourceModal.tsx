...
export function EditTrafficSourceModal({ isOpen, onClose, trafficSource }: EditTrafficSourceModalProps) {
  ...
  useEffect(() => {
    if (trafficSource) {
      form.reset({
        ...
        displayName: trafficSource.displayName,
        isActive: trafficSource.isActive,
      });
      
      if (trafficSource.fields && trafficSource.fields.length > 0) {
        setFields(trafficSource.fields);
      } else {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData & { fields: any[] }) => {
      return await apiRequest(`/api/traffic-sources/${trafficSource.id}`, {
        method: "PATCH",
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to update traffic source",
        variant: "destructive",
      });
    },
  });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={mutation.isPending}>
                {mutation.isPending ? "Updating..." : "Update Traffic Source"}
              </Button>
            </div>
          </form>
        </Form>
